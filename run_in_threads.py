...
def run_algorithm(algo, train_index, test_index, feat, label,opts,staticClassifier=None):
...
def dataProcess(filePath,pre_feature_selection_algorithm='none',feature_drop_rate = 0):
...
def worker(i, pre_feature_selection_algorithm='none', feature_drop_rate = 0):
    ...
    filePath = ''
    if i == 0:
        filePath = r'./data/merged_df12.csv'
    elif i == 1:
        filePath = r'./data/merged_df13.csv'
    elif i == 2:
        filePath = r'./data/merged_df23.csv'
    else:
        print("Invalid input")
        return
    feat,label = dataProcess(filePath,pre_feature_selection_algorithm,feature_drop_rate)
    k = 5  # k-value in KNN
    N = 50  # Number of particles
    T = 50  # Maximum number of iterations
    alpha= 0.90
    opts = {'k': k, 'N': N, 'T': T, 'alpha':alpha}
    results = []
    accuracy_scores = []
    num_feat = []
    ...
    for algo in algorithms:
        ...
        for j in tqdm(range(10), desc=f"Algorithm: {algo}", unit="run"):
            ...
            for fold, (train_index, test_index) in enumerate(kf.split(feat)):
                knnClassifierList.append(np_knn_classifier_for_static_data(feat[train_index],label[train_index],feat[test_index],label[test_index],k))
                train_index_list.append(train_index)     
                test_index_list.append(test_index)           
            for fold in range(len(knnClassifierList)):
                # if fold != 0:
                #     continue
                # try - catch block to handle exceptions
                try:
                    # send queue to the subprocess
                    result = run_algorithm(algo, train_index_list[fold], test_index_list[fold], feat, label,opts,knnClassifierList[fold])
