...
def jfs(xtrain, ytrain, opts):
    ...
    for i in range(N):
        ...
        fit[i, 0] = Fun(xtrain, ytrain, Xbin, opts,np.clip(X[i,:],lb,ub)) if ('runcec' in opts and opts['runcec'] == True) else Fun(xtrain, ytrain, Xbin[i, :], opts)
    ...
    while Iter <= Max_iter:
        ...
        for i in range(N):
            ...
            fit[i, 0] = Fun(xtrain, ytrain, Xbin, opts,np.clip(Xnew[i,:],lb,ub)) if ('runcec' in opts and opts['runcec'] == True) else Fun(xtrain, ytrain, Xbin[i, :], opts)
        ...
        # greedy stragedy
        for i in range(N):
            Xbin = np.where(gwoX[i, :] > thres, 1, 0)
