...
def jfs(xtrain, ytrain, opts):
    ...
    # init for adoptive change way to evaluate
    betterFit= np.ones([N,1],dtype='int')
    ...
    while Iter <= Max_iter:
        ...
        for i in range(N):
            if betterFit[i,0]:                
                ...
                Xnew[i,:] = gwoX[i,:]
        # reset for adoptive change way to evaluate
        betterFit= np.zeros([N,1],dtype='int')
        # Boundary check and evaluation
        Xnew = boundary_check(Xnew, lb, ub)
        # greedy stragedy
        for i in range(N):
            Xbin = np.where(Xnew[i, :] > thres, 1, 0)
            newPopfit[i, 0] = Fun(xtrain, ytrain, Xbin, opts,np.clip(Xnew[i,:],lb,ub))
