...
def jfs(xtrain, ytrain, opts):
    ...
    X = halton_initialization(N, dim, ub, lb)
    ...
    while t < max_iter:
        ...
        # Calculate crowding degree for each individual
        crowding_degree = np.zeros(N)
        for i in range(N):
            distances = np.linalg.norm(X[i] - X, axis=1)
            distances[i] = 0  # Avoid self-distance
            crowding_degree[i] = np.sum(distances) / (N - 1)

        # Apply random perturbation if crowding degree is high
        for i in range(N):
            if crowding_degree[i] < np.mean(crowding_degree):  # If crowding degree is high
                X[i] += np.random.uniform(-0.1, 0.1, dim)
