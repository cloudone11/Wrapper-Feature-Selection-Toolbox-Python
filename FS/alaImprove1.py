    ...
    while Iter <= Max_iter:
        ...
        for i in range(N):
            ...
            if E_rand > 1:
                if np.random.rand() < 0.3:
                    r1 = 2 * np.random.rand(dim) - 1
                    Xnew[i, :] += Xbeta[0, :] + F * RB[i, :] * (r1 * (Xbeta[0, :] - X[i, :]) +
                                                                (1 - r1) * (X[i, :] - X[np.random.randint(N), :]))
                else:
                    r2 = np.random.rand() * (1 + np.sin(0.5 * Iter))
                    Xnew[i, :] += X[i, :] + F * r2 * (Xbeta[0, :] - X[np.random.randint(N), :])
            else:
                if np.random.rand() < 0.5:
                    radius = np.linalg.norm(Xbeta[0, :] - X[i, :])
                    r3 = np.random.rand()
                    spiral = radius * (np.sin(2 * np.pi * r3) + np.cos(2 * np.pi * r3))
                    Xnew[i, :] += Xbeta[0, :] + F * X[i, :] * spiral * np.random.rand()
                else:
                    G = 2 * (np.sign(np.random.rand() - 0.5)) * (1 - Iter / Max_iter)
                    Xnew[i, :] += Xbeta[0, :] + F * G * Levy(dim) * (Xbeta[0, :] - X[i, :])
        Xnew = Xnew / 2
