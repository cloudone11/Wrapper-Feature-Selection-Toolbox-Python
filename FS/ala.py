...
def jfs(xtrain, ytrain, opts):
    ...
    Position = np.zeros((1, dim), dtype='float')
    Score = float('inf')
    ...
    while Iter <= Max_iter:
        ...
        for i in range(N):
            ...
            if E_rand > 1:
                if np.random.rand() < 0.3:
                    ...
                    Xnew[i, :] = Position[0, :] + F * RB[i, :] * (r1 * (Position[0, :] - X[i, :]) +
                ...
                if np.random.rand() < 0.5:
                    ...
                    Xnew[i, :] = Position[0, :] + F * G * Levy(dim) * (Position[0, :] - X[i, :])
        ...
        # Record convergence
        Convergence_curve[0, Iter - 1] = Score
        Iter += 1

    # Best feature subset
    Gbin       = binary_conversion(Position, thres, 1, dim) 
    Gbin       = Gbin.reshape(dim)
    pos        = np.asarray(range(0, dim))    
    sel_index  = pos[Gbin == 1]
    # PositionBin = np.where(Position > thres, 1, 0)
    # sel_index = np.where(PositionBin == 1).flatten()
